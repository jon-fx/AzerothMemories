...
public interface IAccountServices : IComputeService
{
    ...
    [ComputeMethod]
    Task<CheckMemoryPostInfo[]> TrySearchPostsByTime(Session session, long timeStamp, int diffInSeconds, ServerSideLocale locale);
    ...
    [ComputeMethod]
    Task<CheckMemoryResult> TryCheckMemory(Session session, ServerSideLocale locale);
}