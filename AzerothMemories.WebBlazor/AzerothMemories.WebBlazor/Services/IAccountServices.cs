...
public interface IAccountServices
{
    ...
    [ComputeMethod] [Get(nameof(TryReserveUsername) + "/{username}")] Task<bool> TryReserveUsername(Session session, [Path] string username, CancellationToken cancellationToken = default);

    [Post(nameof(TryChangeUsername) + "/{newUsername}")] Task<bool> TryChangeUsername(Session session, [Path] string newUsername, CancellationToken cancellationToken = default);

    [Post(nameof(TryChangeIsPrivate) + "/{newValue}")] Task<bool> TryChangeIsPrivate(Session session, [Path] bool newValue, CancellationToken cancellationToken = default);

    [Post(nameof(TryChangeBattleTagVisibility) + "/{newValue}")] Task<bool> TryChangeBattleTagVisibility(Session session, [Path] bool newValue, CancellationToken cancellationToken = default);
