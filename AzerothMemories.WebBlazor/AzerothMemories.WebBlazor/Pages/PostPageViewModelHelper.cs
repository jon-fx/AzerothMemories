...
public sealed class PostPageViewModelHelper
{
    ...
    public async Task<AccountViewModel?> UpdateAccount(string? accountString)
    {
        ...
        if (accountId > 0)
        {
            AccountViewModel = await _services.ComputeServices.AccountServices.TryGetAccountById(_services.ClientServices.Session, accountId);
        }
        else if (!string.IsNullOrWhiteSpace(accountString))
        {
            AccountViewModel = await _services.ComputeServices.AccountServices.TryGetAccountByUsername(_services.ClientServices.Session, accountString);
        }
    ...
    public async Task<PostViewModel?> UpdatePost(string? postIdString)
    {
        ...
        if (AccountViewModel != null)
        {
            PostViewModel = await _services.ComputeServices.PostServices.TryGetPostViewModel(_services.ClientServices.Session, AccountViewModel.Id, postId, ServerSideLocaleExt.GetServerSideLocale());
    ...
    public async Task<PostCommentPageViewModel?> UpdateComments(string? pageString, string? focusedCommentIdString)
    {
        ...
        CommentReactions = await _services.ComputeServices.PostServices.TryGetMyCommentReactions(_services.ClientServices.Session, PostViewModel.Id);
        PostCommentPageViewModel = await _services.ComputeServices.PostServices.TryGetCommentsPage(_services.ClientServices.Session, PostViewModel.Id, currentPage, focusedCommentId);
